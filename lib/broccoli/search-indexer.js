...
module.exports = class SearchIndexCompiler extends Writer {
  ...
  build() {
    ...
    let index = lunr(function () {
      ...
      this.tokenizer.separator = new RegExp(
        writer.config['ember-cli-addon-docs'].projects[writer.projectName].searchTokenSeparator,
      );
  ...
  *buildDocuments() {
    ...
    for (let filePath of this.listFiles()) {
      ...
      } else if (new RegExp(`${this.projectName}.json`).test(filePath)) {
  ...
  buildTemplateDocument(filePath) {
    let normaliseFilePath = filePath.split(path.sep).join('/'); // normalized both from os native paths ('/' or '\')
    let normaliseInputPath = this.inputPaths[0].split(path.sep).join('/');
    
    let relativePath = normaliseFilePath.replace(normaliseInputPath + '/', '');
    let contents = fs.readJsonSync(normaliseFilePath, 'utf-8');
    ...
    routePath = this.projectName +"/"+ routePath; // 
    ...
    if (routePath && routePath.indexOf('components/') !== 0) {
      return {
        ...
        route: routePath === 'template' ? '' : routePath.replace(/\//g, '.').replace(/\.template$/, ''),
